...
Step 6  收尾汇总 + 矛盾检测提示（CS-001 Gap 修复）
        「━━ 整合完成 ━━
          ✅ 已整合：N 个碎片
          ⚠️ 跳过：M 个
          🔲 仍待处理：K 个
          ...
          ─────────────────────────────
          建议下一步：本次整合涉及 [N] 份 L1 文档的更新。
          若这些文档存在已知张力，建议运行「矛盾检测」确保整合后保持一致：
          「检查有没有问题」→ 触发 cognitive-detect-contradiction」
          
          （此提示为建议性，不阻断；若整合内容较小可跳过）
```
