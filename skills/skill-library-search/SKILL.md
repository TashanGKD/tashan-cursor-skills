...
## ⛔ 七维度强制完成门（project-retrospective B-4 新增）
...
```
□ Dimension 1 已执行（官方组织仓库）
□ Dimension 2 已执行（知名开发者个人仓库）
□ Dimension 3 已执行（Claude Code 专项）
□ Dimension 4 已执行（其他平台专项）
□ Dimension 5 已执行（Topic 搜索）
□ Dimension 6 已执行（交叉引用扩展）
□ Dimension 7 已执行（验证步骤防漏）

IF 任意一项未勾选 → 禁止声称「全量完成」，必须补执行缺失维度
IF 全部勾选 → 才可进入「完成标准」输出
```
