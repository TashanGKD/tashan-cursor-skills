...
- **认知操作统一入口原则**：所有认知操作（初始构建、碎片记录、文档更新）在架构上必须通过主智能体（MainAgent/AgentRunner）作为唯一入口调度执行；禁止在 API 路由层、工具函数层直接调用 workflow 节点或 build_pipeline 等绕过主智能体的硬编码路径。判断违反标准：router 层直接 import 并调用 `start_workflow`/`build_cognition` 等，而非通过 agent runner 发送 skill 指令触发。（来源：engineering-principle-recorder，2026-03-25）
...
### v1.4 — 2026-03-25 — 新增第一性原理「认知操作统一入口原则」

**根因**：项目待办 P-02，通过 engineering-principle-recorder 录入。现有代码中 router 层直接调用 `start_workflow`/`build_cognition`，绕过主智能体调度层，形成双入口架构风险。

**修改内容**：
- 新增：第一性原理节末尾追加「认知操作统一入口原则」

**备份路径**：`history/SKILL_v1.3_20260325_before_cognition-unified-entry.md`
