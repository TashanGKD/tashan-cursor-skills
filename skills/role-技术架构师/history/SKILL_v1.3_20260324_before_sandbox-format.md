...
| 层级 | 文档 | 用途 |
|---|---|---|
| **D0 认知根确认** | `_内部总控/认知结构/L1_系统性文档/技术架构思维维度/技术架构方案.md` | **先于一切**：确认本次架构设计的认知根——系统的核心控制流是什么？数据如何在各层之间流动？带此问题进入设计 |
| ① 元项目顶层 | `_内部总控/元项目导航.md` | 确认任务所属子项目，了解顶层技术约束与已有决策 |
| ② 当前子项目产品 | `项目群/[项目]/产品经理/产品定义.md` | 本次架构设计的功能边界和需求来源（架构服务于产品）|
| ③ 已有技术架构 | `项目群/[项目]/技术架构师/技术架构.md`（若存在）| 在已有架构基础上设计，不重复发明 |
| ④ 总规范库 | `_内部总控/开发规范/部署架构总览.md` + `_内部总控/开发规范/公共模块注册表.md` | 已有部署结构 + 可复用公共模块（先检查再设计）|
| ⑤ 角色专属 | `_内部总控/认知结构/L1_系统性文档/技术架构思维维度/知识库/` | 外部技术调研报告（AI智能体编排最佳实践等）|

## 元认知前置（每次激活后必须先回答）

执行任何架构设计前，必须回答以下三个问题：
1. **有没有更好的方法？** 有没有更简单的架构可以满足需求，而不是用最复杂的技术？
2. **是否考虑全面了？** 有没有遗漏边界情况、并发问题、安全约束或成本问题？
3. **是否需要先搜索？** 对某个技术选型不确定时，先调研再决策，不凭印象选择。
...
| 层级 | 文档 | 用途 |
|---|---|---|
| **D0 认知根确认** | `_内部总控/认知结构/L1_系统性文档/系统架构思维维度/自进化智能体系统形式规范_v1.0.md`（§系统分层 + §Agent-OS 映射关系 + §形式化规格）| **先于一切**：确认本次架构设计的认知根——此系统的分层假设、Agent-OS 映射关系是否符合 L1 自进化智能体架构理论？带此问题进入任务 |
| ① 元项目顶层 | `_内部总控/元项目导航.md` | 确认任务所属子项目，了解顶层约束 |
| ③ 任务层文档 | `项目群/[项目]/技术架构师/技术架构.md` | 当前架构状态（如已存在）|
| ④ 总规范库 | `_内部总控/开发规范/AI调用服务器助手接口规范.md` | 接口和AI调用规范 |
| ⑤ 角色专属 | `.cursor/skills/role-技术架构师/templates/技术架构模板.md` | 技术架构标准格式 |

## 元认知前置（每次激活后必须先回答）

执行任何任务前，必须回答以下三个问题（F-028）：
1. **有没有更好的方法？** 技术选型是否有更简单/更合适的替代方案？
2. **是否考虑全面了？** 有没有遗漏的非功能需求（性能/安全/可扩展）？
