...
❌ 禁止：在浅色白色背景容器里使用深色背景专用配色（text-gray-200、text-blue-200、hover:bg-white/5、bg-black/20），这些在白色背景完全不可见
```

### ⚠️ 背景适配规则（2026-03-22 新增，避免重复踩坑）

**浅色背景容器**（`--bg-container: #FFFFFF` / `bg-white` / `bg-gray-50`）内的组件：
- ✅ 必须使用深色文字：`text-gray-700+`、`border-gray-200`、`bg-white`、实色按钮
- ❌ 禁止：`text-gray-200`、`text-blue-200`（浅色字在白底不可读）
- ❌ 禁止：`hover:bg-white/5`、`bg-black/20`（透明叠加在白底不可见）

**深色背景容器**（导航栏 / 地图画布 `--nav-bg` 等）内的组件：
- ✅ 可以使用：`text-gray-300/200`、`hover:bg-white/10`

> **踩坑案例**：DocTree 和 ProposalCard 使用了深色配色，放入白色 listPanel 后颜色失读。
> 建议为每个组件在注释中标注「适用背景：light / dark」，避免跨容器误用。
...
### v1.2 — 2026-03-22 — 补充背景适配规则（project-retrospective-20260322）

**根因**：DocTree / ProposalCard 使用深色配色，放入白色 listPanel 后颜色失读——`text-gray-200` 在白底不可读，`hover:bg-white/5` 透明叠加在白底不可见。

**修改内容**：
- 新增：「⚠️ 背景适配规则」小节（位于禁止清单之后），区分浅色/深色容器配色要求，并附踩坑案例

**验证结果**：
- 正向验证：在浅色容器中使用深色组件时，AI 应提示违反背景适配规则 → 待验证
- 负向验证：在深色导航栏中使用 `text-gray-300` 不触发警告 → 待验证

**验证状态**：🔵 待验证

**备份路径**：`.cursor/skills/frontend-brand/history/SKILL_v1.1_20260322.md`
