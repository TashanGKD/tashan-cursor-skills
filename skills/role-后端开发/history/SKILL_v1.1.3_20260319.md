...
**根因**：AI 修复 Bug 后没有自动重测的闭合回路——修完代码就算完成，没有触发 verifier 子智能体验证修复是否有效。用户反馈「为什么你没有调用AI工具自己去做产品测试」，暴露了这个结构性缺口。

**修改内容**：
- 新增：「Bug 修复后强制验证闭环」章节（紧接「强制集成测试时机」后）：
  修复 Bug → 构造复现场景 → 调用 /verifier → pass 则更新追踪台 / fail 则继续修（最多3轮）

**验证结果**：
- 正向验证：修复一个 Bug 后，应自动调用 verifier 验证原场景（待真实场景验证）
- 负向验证：正常新功能开发流程（写代码 → 关卡C）不受影响，无额外负担
